...
class VIPWhitelistCheck extends BaseCheck
{
	function check( $files ) {
		...
		$checks = array(
			...
			"/(vip_powered_wpcom)+s?\([^\)]*\)|<a.+href=[\"|']?(http:\/\/en\.wordpress\.com\/vip-hosting\/|http:\/\/vip\.wordpress\.com[\/]?).[\"|']?([^>]+).+Wordpress\.com\sVIP([^<]+)</msiU" => array( "level" => "Warning", "note" => "Attribution link missing or not well formatted" ),
		);
